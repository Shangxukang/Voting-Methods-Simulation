% plurality voting
function winner_probabilities = pluralityvoting(n_parties,n_voters,n_simulation,P,sigma)
winner_counts = zeros(1,n_parties);
    for c = 1:n_simulation
        ...
        for q = 1:size(D_per_index,1)
            Victory(D_per_index(q,1)) = Victory(D_per_index(q,1)) + 1;
        end
        [~,winner] = max(Victory);
    ...
    title('Perceived Preference + Plurality Voting');
    xlabel('Candidate');
